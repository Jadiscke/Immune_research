...
class Antibody():
    ...
    def mutate(self, value):
        ...
        multiplyer = randint(min, max)/1000.0
        self.paratope *= multiplyer
        self.update_affinity()
        print '\t{} * [{}~{}] = {} --> {} == {}'.format(x, min/1000.0, max/1000.0, multiplyer, self.paratope, self.affinity)
    
    def update_affinity(self):
        x = self.paratope
        self.affinity = -1*(x**4) + 10 * x ** 3 - 2 * x**2 + 50
...
def generate_antibodies(n, ag):
    antibodies = []
    for num in range(n):
        paratope = randint(1, 10000)/1000.0
